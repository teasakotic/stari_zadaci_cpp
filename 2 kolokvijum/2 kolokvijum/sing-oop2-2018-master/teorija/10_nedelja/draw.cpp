...
Canvas::Canvas(){
	...
	rect.G = 200;
	rect.B = 0;
	pravougaonici.push_back(rect);

	Rectangle rect1 = Rectangle(200, 50, 280, 200);
	rect1.R = 0;
	rect1.G = 0;
	rect1.B = 200;
	pravougaonici.push_back(rect1);
...
bool Canvas::loadFromFile(string path)
{
	...
	while (getline(myfile, linija))
	{
		...
		linije.push_back(Line(x1, y1, x2, y2));
	}
	myfile.close();
		...
		case SDL_MOUSEBUTTONUP:
			switch(state){
				...
				case 1:
					...
					Rectangle rect = Rectangle(pre.x, pre.y, event.button.x, event.button.y);
					rect.R = 100;
					rect.G = 200;
					rect.B = 0;
					pravougaonici.push_back(rect);
