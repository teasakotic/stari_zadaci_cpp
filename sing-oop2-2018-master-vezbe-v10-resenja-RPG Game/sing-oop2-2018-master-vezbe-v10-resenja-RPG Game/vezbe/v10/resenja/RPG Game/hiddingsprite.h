#ifndef HIDDINGSPRITE_H_INCLUDED
#define HIDDINGSPRITE_H_INCLUDED

class HiddingSprite : public Sprite { ///nova klasa

public:
    HiddingSprite(SpriteSheet* ss) : Sprite(ss) {}
    void move() {
    ...
    void setState(short int state){
        if (this->state == HIDDING && state == 0){
            this->state = state;
        }
    }
};

#endif // HIDDINGSPRITE_H_INCLUDED
